...
class OpenAIReader:
    def __init__(self, model_name="deepseek-r1", base_url=None):
...
def run_pipeline(retriever_type="dense", reader_type="openai"):
    # 加载 PubMedQA 数据集
    pubmedqa_file_path = r'ori_pqau.json'  # 替换PubMedQA 数据文件路径
    ...
    if reader_type == "hf":
        ...
        reader = OpenAIReader(model_name="deepseek-r1")
    else:
